...
def select_mode(key, mode):
    ...
    if 48 <= key <= 57:  # 0 ~ 9
        number = key - 48
    if key == 110:  # n #pause logging
        mode = 0
    if key == 107:  # k # keypoint
        mode = 1
    if key == 104:  # h # history
        mode = 2
    return number, mode
...
def draw_info_text(image, brect, handedness, hand_sign_text = None,
    ...
    #if hand_sign_text != "":
    if hand_sign_text is not None:
        info_text = info_text + ':' + hand_sign_text
    cv.putText(image, info_text, (brect[0] + 5, brect[1] - 4),
               cv.FONT_HERSHEY_DUPLEX, 0.6, (0, 0, 0), 1, cv.LINE_AA)

    if finger_gesture_text is not None:
